            ...
            for m in model.modules():
                if 'Mlp' in str(m) and 'Attention' not in str(m):
                    ...
                    for H1 in range(X_.shape[-1]):
                        # 相关性分数 max
                        rel.append(relevance(X_[:,:,H1], F.softmax(output, dim=-1)).item())  
                        # 冗余度分数 min
                        red.append(redundancy(X_[:,:,H1], X_mean).item())
                        # 激活值分数 max
                        act.append(np.sum(X_[:,:,H1].abs().detach().cpu().numpy(), axis=(0, 1)))
                    # 归一化处理
                    rel = np.array(rel)
                    ...
                    # 综合分数
                    temp = (0.1*rel + 0.1*red + 0.8*act).tolist()
            ...
            for m in model.modules():
                if 'Attention' in str(m) and 'Mlp' not in str(m):
                    ...
                    for idx_head in range(m.num_heads):
                        head_mean.append(torch.mean(X_head[:,:,idx_head,:], dim=-1))
                    ...
                    for H1 in range(X_.shape[2]):
                        ...
                        for H2 in range(m.num_heads):
                            if H1 not in np.arange(H2*m.head_dim, (H2+1)*m.head_dim):
                                redundancy_count += redundancy(X_[:,:,H1], head_mean[H2]).item()
                        redundancy_count /= (len(head_mean)-1)
                        redundancy_list.append(redundancy_count)
                        # 激活值分数 max
                        activeness_count = np.sum(X_[:,:,H1].abs().detach().cpu().numpy(), axis=(0, 1))
                        activeness_list.append(activeness_count)
                    # 归一化处理
                    ...
                    activeness_list = np.array(activeness_list)
                    activeness_list = (activeness_list - np.min(activeness_list)) / (np.max(activeness_list) - np.min(activeness_list))
                    # 综合分数
                    temp = (0.8*relevance_list + 0.1*redundancy_list + 0.1*activeness_list).tolist()
